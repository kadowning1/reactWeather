...
export const LocationSearch = ({ onSearch, hasSearched, setGeolocation }: LocationSearchProps) => {
    ...
    <div className='my-6'>
      <form className='flex flex-col justify-center items-center'>
        ...
        <div className="relative justify-center lg:w-2/3 sm:w-full">
          ...
          <button type="submit" className="cursor-pointer text-white absolute right-2.5 bottom-2.5 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 bg-black opacity-0 md:opacity-100 lg:w-1/6" onClick={addLocation} disabled={disableSearch}>Search</button>
        </div>
        <div className='relative justify-center lg:w-1/2 sm:w-full'>
          <button type="submit" className="cursor-pointer mt-4 text-white focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 bg-black opacity-100 md:opacity-0 lg:w-1/6" onClick={addLocation} disabled={disableSearch}>Search</button>
        </div>
      </form>
    </div>
  );
